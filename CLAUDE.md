...
- **Odds market-ID correction (2026-06-18).** The odds parser had goals on `market_id 7` (a thin goals market with NO 1.5 line → `goals_over_15_odd` never landed) and corners on `market_id 45` (which is actually **corners odd/even**, not totals). Verified from `raw_odds_json`: the real markets are **goals = `market_id 80`** (has 1.5/2.5/3.5 + Asian lines) and **corners = `market_id 67`** (lines ~7.5–12.5). Hit rates were never affected (score/corner-settled), only displayed odds + the cross-signal. Fix: `scripts/rederive_odds.py` re-derives goals(80)/corners(67) over+under from the stored `raw_odds_json` into the `*_over_*`/`*_under_*` columns (idempotent, additive); wired as a post-step in `run_daily.ps1` and appended (guarded) to `fetch_upcoming.py`. NOTE: `raw_odds_json` only exists from ~2026-06-08, so only recent fixtures can be re-derived; the cross-signal needs calendar time to accumulate. The legacy inline `extract_odds` (mkt 7/45) is left in place but superseded by the re-derive. `fetch_upcoming` inline `_zone` still uses STALE V3 cutoffs (2.70/3.40/4.10/4.80) — flagged, not yet fixed.
