    ...
    if existing:
        conn.execute("""
            UPDATE fixtures SET
                ...
                draw_zone=?, bts_pocket=?, updated_at=?, odds_updated_at=?, raw_odds_json=?
            WHERE sportmonks_id=?
        """, (internal_league_id, kickoff_utc, home_odd, draw_odd, away_odd,
              btts_yes, btts_no, g15, g25, g35, c75, c85, c95,
              fx_zone, fx_bts, now_ts, now_ts, json.dumps(fx.get("odds") or []), sm_id))
        updated += 1
    else:
        conn.execute("""
            INSERT INTO fixtures
                (sportmonks_id, league_id, tier, date, status,
                 ...
                 goals_over_15_odd, goals_over_25_odd, goals_over_35_odd,
                 corners_over_75_odd, corners_over_85_odd, corners_over_95_odd,
                 draw_zone, bts_pocket, created_at, updated_at, odds_updated_at, raw_odds_json)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        """, (
