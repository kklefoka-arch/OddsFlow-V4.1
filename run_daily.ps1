...
Write-Host "=== db_healthcheck (integrity + backup) ===" -ForegroundColor Cyan
python db_healthcheck.py 2>&1 | Tee-Object -FilePath "$logDir\db_healthcheck_$stamp.log"

Write-Host "`n=== sync_leagues (active set from live subscription) ===" -ForegroundColor Cyan
python sync_leagues.py 2>&1 | Tee-Object -FilePath "$logDir\sync_leagues_$stamp.log"
